...
class KeyvValkey extends Hookified implements KeyvStorageAdapter {
	 ...
	 * @returns {Promise<KeyvRawResult<Value>>} The stored data if found, or `undefined` if the key does not exist.
	 */
	 ...
	 * @returns {Promise<Array<KeyvRawResult<Value | undefined>>>} An array of stored data in the same order as the input keys.
	...
	public async getMany<Value>(keys: string[]): Promise<Array<KeyvRawResult<Value | undefined>>> {
		...
		if (this.isCluster()) {
			...
			const resultMap = new Map<string, KeyvRawResult<Value | undefined>>();
			...
			return resolvedKeys.map((k) => resultMap.get(k) as KeyvRawResult<Value | undefined>);
		}
