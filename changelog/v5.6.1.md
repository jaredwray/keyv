...
(`.github/workflows/release.yaml`) run from the `v5` branch after this release
merges — it stages each package whose local version is ahead of npm (npm staged
publishing with provenance, via OIDC trusted publishing) under the dist-tag the
release script computes (`latest` while the v5 line still owns it); a maintainer
then approves the staged versions with 2FA. See the [changelog README](./README.md).
