...
- Handles all JSON types plus `Buffer` via `@keyv/serialize`
- Supports namespaces
...
Create a new Keyv instance, passing your connection string if applicable. Keyv will automatically load the correct storage adapter. Choose from `redis`, `mongodb`, `sqlite`, `postgresql`, `mysql`, `memcache`, or `etcd`. Here are a few examples:
...
// once you have the storage adapter, you can use it like this
...
You can also pass in the storage adapter via options like this example where we also specify the `namespace` and `ttl`:

```js
import Keyv from 'keyv';
import KeyvRedis from '@keyv/redis';
const keyvRedis = new KeyvRedis('redis://user:pass@localhost:6379');
const keyv = new Keyv({ store: keyvRedis, namespace: 'cache', ttl: 1000 });
```
...
```js
import Keyv from 'keyv';
import myAdapter from './my-storage-adapter';
...
### new Keyv([storage_adapter], [options])
...
Type: `KeyvStorageAdapter || Map` <br />
Default: `undefined`
...
Please see the [CONTRIBUTING.md](CONTRIBUTING.md) and [CODE_OF_CONDUCT.md](CODE_OF_CONDUCT.md) files for more information. Once you are ready to contribute, please follow the guidelines at [Keyv's main repository](https://github.com/jaredwray/keyv).
