...
**Backward compatibility — legacy adapters keep working.** If an adapter does **not** advertise `capabilities.expires`, Keyv treats it as a v5-style adapter and automatically wraps it in `KeyvBridgeAdapter`, which converts the absolute `expires` back to a relative `ttl` before delegating to your `set`/`setMany`. So existing third-party adapters continue to function without code changes, including adapters compiled to an older target such as ES2016, whose methods return promises without being `async` functions. If such an adapter has none of `getMany`, `setMany`, `hasMany`, `deleteMany`, `disconnect`, or `iterator`, Keyv can't tell it from a synchronous `Map`, and its `get`, `has`, and `set` throw; pass `new KeyvBridgeAdapter(store)` instead. Upgrading to the v6 contract is still recommended: it removes the bridge conversion and eliminates the silent-expiry bug described above.
