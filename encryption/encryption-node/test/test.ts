...
import KeyvEncryptionNode from "../src/index.js";

describe("KeyvEncryptionNode", () => {
	const secret = "my-secret-key";

	describe("default aes-256-gcm", () => {
		it("should encrypt and decrypt a string", () => {
			const encryption = new KeyvEncryptionNode({ key: secret });
		...
		it("should fail to decrypt with a different key", () => {
			const encryption1 = new KeyvEncryptionNode({ key: "key-one" });
			const encryption2 = new KeyvEncryptionNode({ key: "key-two" });
	...
	describe("key handling", () => {
		it("should accept a string key and derive via SHA-256", () => {
			const encryption = new KeyvEncryptionNode({ key: "any-length-string-works" });
			const data = "test";
		...
		it("should accept a 32-byte Buffer key", () => {
			...
			const encryption = new KeyvEncryptionNode({ key: bufferKey });
		...
		it("should throw if Buffer key has wrong length for aes-256-gcm", () => {
			...
			expect(() => new KeyvEncryptionNode({ key: badKey })).toThrow("Key must be 32 bytes");
		});
	});
	...
	describe("custom algorithms", () => {
		it("should work with aes-128-gcm", () => {
			...
			const encryption = new KeyvEncryptionNode({ key: bufferKey, algorithm: "aes-128-gcm" });
		...
		it("should work with aes-128-gcm using string key", () => {
			const encryption = new KeyvEncryptionNode({ key: "my-key", algorithm: "aes-128-gcm" });
		...
		it("should work with chacha20-poly1305", () => {
			const bufferKey = randomBytes(32);
			const encryption = new KeyvEncryptionNode({ key: bufferKey, algorithm: "chacha20-poly1305" });
		...
		it("should work with aes-256-cbc (non-AEAD)", () => {
			const bufferKey = randomBytes(32);
			const encryption = new KeyvEncryptionNode({ key: bufferKey, algorithm: "aes-256-cbc" });
		...
		it("should throw for unsupported algorithm", () => {
			expect(() => new KeyvEncryptionNode({ key: "test", algorithm: "invalid-algorithm" })).toThrow(
	...
	describe("encoding option", () => {
		it("should support hex encoding", () => {
			const encryption = new KeyvEncryptionNode({ key: secret, encoding: "hex" });
	...
	describe("Keyv integration", () => {
		...
		it("should work with Keyv and custom algorithm", async () => {
			const encryption = new KeyvEncryptionNode({ key: secret, algorithm: "aes-128-gcm" });
			const keyv = new Keyv({ encryption });
